...
- published 93건. pending 0건. 백로그 전량 처리 완료. 2026-08-06 통계 수집기(m.blog.naver.com 모바일 파싱) 복구 완료.
- 2026-08-08 표 발행 실험 완료: 네이버가 클립보드 HTML을 평문으로 내리는 문제를 확인. 이후 Markdown 표를 모바일용 텍스트 카드로 정규화해 385번 글에서 원시 `|---|` 없이 표시되는 것을 확인.
...
1. 방문자 통계 수집 복구 완료 (2026-08-06): 모바일 네이버 블로그 URL 파싱 방식으로 daily_stats 자동 갱신 재개.
2. 네이버 계정 `active` 복구 완료(2026-08-05). 세션 재만료 시 Jarvis 알림 후 `../venv/bin/python login_naver.py` headed 실행으로 수동 확인.
3. Qwen API 키 로테이션 권장: 과거 `.env.md` 평문 노출 이력(파일은 2026-08-05 삭제). Alibaba 콘솔에서 키 재발급 후 `.env`만 갱신.
4. Python 3.9 EOL 및 LibreSSL 경고가 있어 Python 3.11+ 런타임으로 이전 권장.
5. 대시보드가 LAN 전체(`0.0.0.0`)에 인증 없이 노출됨. Basic Auth/CSRF 적용 필요.
6. LAN IP가 DHCP로 바뀌므로 고정 주소 또는 로컬 호스트명 도입 검토.
